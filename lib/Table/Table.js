...
class Table extends Component {
    state = {
        fieldOrder: '',
        order: 'desc',
        iconOrder: 'sort-amount-down',
        data: [],
    }

    componentDidMount() {
        const { props: { headers, data } } = this;

        this.setState({
            fieldOrder: headers[0],
            data,
        });
    }

    componentDidUpdate(prevProps) {
        const { props: { data, headers } } = this;
        if (prevProps.data.length !== data.length) {
            this.orderBy(headers[0]);
        }
    ...
    orderBy = field => {
        ...
        this.setState({
            ...
            data,
            fieldOrder: field,
            iconOrder: state.order === 'desc' ? 'sort-amount-up' : 'sort-amount-down',
        })
    }


    render() {
        const { state, props: { headers, actionPositions } } = this;
        ...
        if (state.data.length) {
            Object.entries(state.data[0]).forEach(item => {
                if (item[1]['width']) {
        ...
        let columnWidth = (100 - totalWidthSetted) / (headers.length - totalItemsWithWidthSetted);
            ...
            <ThemeProvider theme={theme}>
                <ListTableHeader>
                    {headers.map((item, index) => {
                        ...
                        const width = (state.data.length && state.data[0][item] && state.data[0][item].width) || columnWidth;
                        const removeTitle = state.data.length && state.data[0][item] && state.data[0][item].noTitle;
                        const noSort = state.data.length && state.data[0][item] && state.data[0][item].noSort;
                        ...
                        return (
                            <ListTableHeaderItem
                                ...
                                onClick={removeTitle || noSort ? () => {} : () => this.orderBy(item)}
                            >
                                {!removeTitle &&
                                    <>
                                        {item}
                                        {item === state.fieldOrder && !isAction && !noSort &&
                                            <IconOrderBy>
                                                <FontAwesomeIcon icon={['fal', state.iconOrder]} />
                                            </IconOrderBy>}
                                    </>
                ...
                <ContainerPadding>
                    {state.data.map((row, index, array) => {
                        return (
